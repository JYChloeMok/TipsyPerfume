...
COMMENT ON COLUMN "TB_PRODUCT"."PDT_SHIPPING" IS '브랜드는 배송비(무료배송 0원) / 펀딩은 배송일 (펀딩확정 N일 뒤 배송 예정입니다)';

COMMENT ON COLUMN "TB_PRODUCT"."PDT_COUNT" IS '상품 게시글 조회수';

COMMENT ON COLUMN "TB_PRODUCT"."PDT_STATUS" IS '판매 노출 여부 / Y : 판매중, N : 판매중단';
...
COMMENT ON COLUMN "TB_ORDER_PRODUCT"."ORDER_DETAIL_NO" IS '주문-상품 번호';
...
CREATE TABLE "TB_PAY" (
	...
	"PAY_SHIPPING"	NUMBER	DEFAULT 0	NULL
);
...
COMMENT ON COLUMN "TB_PAY"."PAY_SHIPPING" IS '배송비 (무료배송 0 )';
...
CREATE TABLE "TB_PAY_KAKAO" (
	"PAY_KAKAO_NO"	NUMBER		NOT NULL,
	"USER_EMAIL"	VARCHAR2(50)		NOT NULL,
	"TID"	VARCHAR2(20)		NULL,
	"ITEM_CODE"	VARCHAR2(200)		NULL
);

COMMENT ON COLUMN "TB_PAY_KAKAO"."PAY_KAKAO_NO" IS '카카오페이 임시주문서 번호';

COMMENT ON COLUMN "TB_PAY_KAKAO"."USER_EMAIL" IS '회원 이메일(UNIQUE 제약조건이 있음)';

COMMENT ON COLUMN "TB_PAY_KAKAO"."TID" IS '부여된 TID';

COMMENT ON COLUMN "TB_PAY_KAKAO"."ITEM_CODE" IS '장바구니 번호(CART_NO 구분자 , 로 이어 공백없이 문자열로 저장)';
...
ALTER TABLE "TB_ORDER_PRODUCT" ADD CONSTRAINT "FK_PDT_OPTION_TO_ORDER_PRODUCT" FOREIGN KEY (
	"PDT_OPTION_NO"
)
REFERENCES "TB_PRODUCT_OPTION" (
	"PDT_OPTION_NO"
...
ALTER TABLE "TB_PAY_KAKAO" ADD CONSTRAINT "PK_TB_PAY_KAKAO" PRIMARY KEY (
	"PAY_KAKAO_NO"
);
