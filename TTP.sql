...
CREATE TABLE "TB_PRODUCT" (
	...
	"PDT_NAME"	VARCHAR2(500)		NOT NULL,
	"PDT_DATE"	DATE	DEFAULT SYSDATE	NULL,
	"PDT_INTRO"	VARCHAR2(2000)		NOT NULL,
	"PDT_DESCRIPTION"	VARCHAR2(2000)		NOT NULL,
	"PDT_INGREDIENT"	VARCHAR2(2000)		NOT NULL,
	"PDT_SHIPPING"	NUMBER		NOT NULL,
	"PDT_COUNT"	NUMBER	DEFAULT 0	NULL,
	"PDT_STATUS"	CHAR(1)	DEFAULT 'Y'	NULL
);
...
COMMENT ON COLUMN "TB_PRODUCT"."PDT_SHIPPING" IS '배송비(무료배송 0원)';
...
CREATE TABLE "TB_CART" (
	...
	"CART_QUANTITY"	NUMBER	DEFAULT 1	NULL
);
...
COMMENT ON COLUMN "TB_CART"."PDT_OPTION_NO" IS '옵션 번호';

COMMENT ON COLUMN "TB_CART"."CART_QUANTITY" IS '상품 수량';
...
COMMENT ON COLUMN "TB_REVIEW"."PDT_OPTION_NO" IS '상품 옵션 번호';
...
CREATE TABLE "TB_BOARD" (
	...
	"BOARD_CTGY_CODE"	CHAR(5)		NOT NULL,
...
CREATE TABLE "TB_ORDER" (
	...
	"ORDER_ARRIVAL_DATE"	DATE		NULL,
	"ORDER_MESSAGE"	VARCHAR2(90)		NULL
);

COMMENT ON COLUMN "TB_ORDER"."ORDER_NO" IS '주문서 번호';

COMMENT ON COLUMN "TB_ORDER"."USER_NO" IS '주문한 회원번호';

COMMENT ON COLUMN "TB_ORDER"."RECEIVER_NO" IS '주소록 번호';

COMMENT ON COLUMN "TB_ORDER"."PAYMENT_NO" IS '결제 번호';

COMMENT ON COLUMN "TB_ORDER"."ORDER_DATE" IS '주문일(년월일시분초)';

COMMENT ON COLUMN "TB_ORDER"."ORDER_ARRIVAL_DATE" IS '배송완료일(년월일시분초)';
...
CREATE TABLE "TB_PRODUCT_OPTION" (
	...
	"PDT_OPTION_PRICE"	NUMBER		NOT NULL,
	"PDT_OPTION_STOCK"	NUMBER	DEFAULT 0	NULL,
	"PDT_OPTION_STATUS"	CHAR(1)	DEFAULT 'Y'	NULL
);
...
COMMENT ON COLUMN "TB_PRODUCT_OPTION"."PDT_OPTION_PRICE" IS '옵션별 판매 가격';

COMMENT ON COLUMN "TB_PRODUCT_OPTION"."PDT_OPTION_STOCK" IS '옵션별 상품 재고';

COMMENT ON COLUMN "TB_PRODUCT_OPTION"."PDT_OPTION_STATUS" IS '옵션별 판매중 상태 / Y : 판매중, N : 판매중단';
...
COMMENT ON COLUMN "TB_ORDER_PRODUCT"."ORDER_NO" IS '주문서 번호';

COMMENT ON COLUMN "TB_ORDER_PRODUCT"."PDT_OPTION_NO" IS '옵션 번호';

COMMENT ON COLUMN "TB_ORDER_PRODUCT"."ORDER_QUANTITY" IS '상품 수량';

CREATE TABLE "TB_PAYMENT" (
	"PAYMENT_NO"	NUMBER		NOT NULL,
	"PAID_AMOUNT"	NUMBER		NULL,
	"APPLY_NUM"	VARCHAR2(100)		NULL,
	"IMP_UID"	VARCHAR2(30)		NULL,
	"MERCHANT_UID"	VARCHAR2(100)		NOT NULL,
	"PG_TID"	VARCHAR2(200)		NULL,
	"PG_PROVIDER"	VARCHAR2(50)		NULL,
	"PAY_METHOD"	VARCHAR2(50)	DEFAULT 'card'	NULL,
	"PAID_AT"	VARCHAR2(50)		NULL,
	"PAY_STATUS"	VARCHAR2(30)	DEFAULT 'ready'	NULL,
	"CUSTOM_DATA"	VARCHAR2(1000)		NULL
);

COMMENT ON COLUMN "TB_PAYMENT"."PAYMENT_NO" IS '결제 번호';

COMMENT ON COLUMN "TB_PAYMENT"."PAID_AMOUNT" IS '결제금액';

COMMENT ON COLUMN "TB_PAYMENT"."APPLY_NUM" IS '신용카드 승인 번호';

COMMENT ON COLUMN "TB_PAYMENT"."IMP_UID" IS '포트원 고유 결제번호';

COMMENT ON COLUMN "TB_PAYMENT"."MERCHANT_UID" IS '주문서 UID (가맹점 주문번호)';

COMMENT ON COLUMN "TB_PAYMENT"."PG_TID" IS 'PG사 거래번호';

COMMENT ON COLUMN "TB_PAYMENT"."PG_PROVIDER" IS 'PG사 구분코드 (간편결제 EMB_PG_PROVIDER)';

COMMENT ON COLUMN "TB_PAYMENT"."PAY_METHOD" IS '결제수단 구분코드 (신용카드 card, 가상계좌 vbank)';

COMMENT ON COLUMN "TB_PAYMENT"."PAID_AT" IS '결제승인시각';

COMMENT ON COLUMN "TB_PAYMENT"."PAY_STATUS" IS '결제상태 (ready : 진행 도중 중단 등 미결제상태, paid : 결제완료, failed : 결제실패)';

COMMENT ON COLUMN "TB_PAYMENT"."CUSTOM_DATA" IS '가맹점 임의 지정 데이터';
...
CREATE TABLE "TB_RECEIVER" (
	...
	"PRIMARY_STATUS"	VARCHAR2(1)	DEFAULT 'P'	NULL
);
...
COMMENT ON COLUMN "TB_RECEIVER"."PRIMARY_STATUS" IS '배송지 종류 구분 / 기본(등록 배송지) : P, 기타(등록배송지) :  E, 1회성 배송지 : T, 현재 사용하지 않는 배송지 : N';
...
ALTER TABLE "TB_ORDER_PRODUCT" ADD CONSTRAINT "PK_TB_ORDER_PRODUCT" PRIMARY KEY (
	"ORDER_NO",
	"PDT_OPTION_NO"
);

ALTER TABLE "TB_ORDER_PRODUCT" ADD CONSTRAINT "FK_ORDER_TO_ORDER_PRODUCT" FOREIGN KEY (
	"ORDER_NO"
)
REFERENCES "TB_ORDER" (
	"ORDER_NO"
...
ALTER TABLE "TB_ORDER_PRODUCT" ADD CONSTRAINT "FK_PDT_OPT_TO_ORDER_PDT" FOREIGN KEY (
	"PDT_OPTION_NO"
)
...
ALTER TABLE "TB_ORDER" ADD CONSTRAINT "FK_PAYMENT_TO_ORDER" FOREIGN KEY (
	"PAYMENT_NO"
)
...
ALTER TABLE "TB_CART" ADD CONSTRAINT "FK_PRODUCT_OPTION_TO_CART_" FOREIGN KEY (
	"PDT_OPTION_NO"
)
REFERENCES "TB_PRODUCT_OPTION" (
	"PDT_OPTION_NO"
